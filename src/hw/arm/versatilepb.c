...
static int do_arm_linux_init(Object *obj, void *opaque)
{
	...
	bool secure_boot = false;
        ...
        if (albifc->arm_linux_init) {
            albifc->arm_linux_init(albif, secure_boot);
        }
    }
    return 0;
}
...
#define WRITE_WORD(p, value) do { \
    ...
    hwaddr p = MEM_START_ADDRESS + KERNEL_ARGS_ADDR;;
...
//    cpu_set_pc(cs, KERNEL_LOAD_ADDRESS);
    ...
    WRITE_WORD(p, MEM_START_ADDRESS);
    /* ATAG_END */
    WRITE_WORD(p, 0);
    WRITE_WORD(p, 0);
}
...

{
    ...
    static uint32_t ram0_memory[KERNEL_LOAD_ADDRESS/sizeof(uint32_t)] = {
	    ...
	    0x00000183, //versatile_pb board id
	    0x00000100, //kernel arg address
	    KERNEL_LOAD_ADDRESS, //kernel load address
    };
    int i;
    for (i = 0; i < 7; i++)
	    ram0_memory[i] = tswap32(ram0_memory[i]);

    memory_region_init_ram_ptr(ram0, NULL, "ram0", sizeof(ram0_memory), ram0_memory);
    memory_region_add_subregion(sysmem, MEM_START_ADDRESS, ram0);

    memory_region_init_ram_ptr(ram1, NULL, "ram1", ram_len, ram_mem);
    memory_region_add_subregion(sysmem, KERNEL_LOAD_ADDRESS, ram1);

    ram_size = machine->ram_size = ram_len + KERNEL_LOAD_ADDRESS;
#endif
