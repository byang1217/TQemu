...
{
    tcg_target_ulong value = *(tcg_target_ulong_align1 *)(*tb_ptr);
    *tb_ptr += sizeof(value);
    return value;
}
...
{
    uint32_t value = *(uint32_t_align1 *)(*tb_ptr);
    *tb_ptr += sizeof(value);
    return value;
}
...
{
    int32_t value = *(int32_t_align1 *)(*tb_ptr);
    *tb_ptr += sizeof(value);
    return value;
}
...
uintptr_t tcg_qemu_tb_exec(CPUArchState *env, uint8_t *tb_ptr)
{
	...
	if (temp_log_enable){
		...
		if (prev_op_len > 0) {
			...
			for (i = 0; i < prev_op_len; i++)
				printf("%02x ", prev_tb_ptr[i]);
			printf("\n");
		}
#endif
		printf("[%x]: %s\n", (int)(tb_ptr - org_tb_ptr), def->name);
		prev_tb_ptr = tb_ptr;
        ...
        case INDEX_op_exit_tb:
            next_tb = *(uint64_t_align1 *)tb_ptr;
            goto exit;
            break;
